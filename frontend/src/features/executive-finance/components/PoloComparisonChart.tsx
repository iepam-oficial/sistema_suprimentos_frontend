...
interface PoloComparisonChartProps {
...
const METRIC_LABELS: Record<PoloMetric, string> = {
  [PoloMetric.DESPESAS]: 'Despesas',
  [PoloMetric.COMPRAS]: 'Compras',
  [PoloMetric.ECONOMIA]: 'Economia',
  [PoloMetric.PATRIMONIO]: 'Patrimônio',
};
    ...
    <Box borderWidth="1px" borderColor={borderColor} borderRadius="md" bg={cardBg} p={3}>
      <HStack justify="space-between" mb={2}>
        ...
        <Select
          ...
          bg={selectBg}
          value={metric}
          onChange={(e) => onMetricChange(e.target.value as PoloMetric)}
          aria-label="Métrica do comparativo entre polos"
        >
          {Object.values(PoloMetric).map((value) => (
      ...
      {loading && !data ? (
        ...
        <Box
          ...
          fontSize="sm"
        >
          Sem dados de polos no período selecionado
        </Box>
      ) : (
